...
img1 = cv.imread(imgname1) # Reading the image file 
...
while (True): # Looping through all the frames
    ...
    ret, frame = cap.read()
    frame1 = copy.deepcopy(frame) # Making a copy of the frame
            ...
            if not(0 in list4): # If no '0' is present in list4 i.e. all the zeroes elements are replaced
                ...
                for m in range(0,200): # Looping over the height of world frame
                    for n in range(0,200): # Looping over the width of world frame
                        x1, y1, z1 = np.matmul(H1,[m,n,1]) # Calculating camera frame coordinates with respect to world frame coordinates
                        if (int(y1/z1) < 1080 and int(y1/z1) > 0) and (int(x1/z1) < 1920 and int(x1/z1) > 0):
                            im_out[m][n] = threshold[int(y1/z1)][int(x1/z1)] # Creating a new image from the camera frame 
                ...
                if flag: # If id is identifiable        
                    ...
                    img1 = cv.resize(img1, (200, 200)) # Resizing the given template into 200x200 resolution
                    H1 = homography(list3, [[0,0],[0,199],[199,0],[199,199]]) # Calculating homography between the camera frame(AR Tag) and world frame(Lena Image)
                    H1inv = np.linalg.inv(H1) # Calculating inverse of the homographic matrix  
            
                    for j in range(0,200): # Looping over the height of the template image
                        for k in range(0,200): # Looping over the width of the template image
                            x2, y2, z2 = np.matmul(H1inv,[j,k,1]) # Calculating camera frame coordinates with respect to the world coordinates
                            if (int(y2/z2) < 1080 and int(y2/z2) > 0) and (int(x2/z2) < 1920 and int(x2/z2) > 0): 
                                frame1[int(y2/z2)][int(x2/z2)] = img1[j][k] # Replacing camera frame coordinates with the world coordinates
                                 
    #break   
    cv.imshow("Superimposing_Lena", frame1) # Showing a new window with Lena Superimposed

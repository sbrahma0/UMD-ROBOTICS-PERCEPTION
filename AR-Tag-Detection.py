    ...
    if list2 != []: # If list2 is not empty
        for i in range(0,len(list2)): # Looping over all the groups of corners identified
            ...
            if not(0 in list4): # If no '0' is present in list4 i.e. all the zeroes elements are replaced 
                cv.circle(frame, (list4[0][0],list4[0][1]), 5, (0,0,255)) # Create a ring of red color on the top left corner in the frame
                cv.circle(frame, (list4[1][0],list4[1][1]), 5, (0,255,0)) # Create a ring of green color on the top right corner in the frame
                cv.circle(frame, (list4[2][0],list4[2][1]), 5, (255,0,0)) # Create a ring of blue color on the bottom left corner in the frame
                cv.circle(frame, (list4[3][0],list4[3][1]), 5, (0,200,255)) # Create a ring of yellow color on the bottom right corner in the frame 
                ...
                #im_out = cv.warpPerspective(threshold, H, (200,200)) # Generating AR Tags from frame
                flag, angle_value, identity = calculateTagId(im_out) # calling calculateTagId
                
                if flag: # If id is identifiable
                    # Id is printed in the frame near the top left corner of the corresponding AR-Tag
                    cv.putText(frame, str(int(identity)), (list4[0][0]-50, list4[0][1]-50), fontFace = cv.FONT_ITALIC, thickness=2, fontScale = 1, color= (0,0,255))
                cv.imshow("AR_tag" + str(i), im_out)
            
    #break  
    cv.imshow("Tag_Identification", frame) # Showing a new window with tag identification
